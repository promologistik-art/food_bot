...
def extract_product_data(product: dict) -> dict:
    """Извлекает данные продукта из ответа DeepSeek"""
    return {
...
async def handle_message(message: types.Message):
    ...
    # Отправляем ответ с таблицей
    if user_text:
        await message.answer(user_text + "\n\n" + format_daily_stats(stats), parse_mode="Markdown")
    else:
        # fallback если нет таблицы
        response = "📋 *Добавлено:*\n\n"
        for p in products:
            pd = extract_product_data(p)
            response += f"• {pd['name']} — {pd['weight_grams']:.0f}г — {pd['calories']:.0f} ккал\n"
